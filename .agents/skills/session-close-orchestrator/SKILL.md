...
1. Read `.Codex/current-task` if it exists. If it points to a valid file, go
   to step 2.
2. If `.Codex/current-task` is missing or the file it points to does not exist,
   ...
   a. Classify the work from the session conversation:
      ...
      - `owner`: the best-fit steward skill from AGENTS.md's skill table
...
an up-to-date `.Codex/current-task`.
...
1. `.Codex/current-task` — pointing to the current task card (Step 0)
...
When invoked:
- Use `Read` to check `.Codex/current-task` and any existing task card.
- Use `Write` to create or update the task card and `.Codex/current-task`.
...
- Use `TaskCreate` / `TaskUpdate` to track pipeline steps if the session is
  complex enough.
- For the experience/skill/agent evaluation, check Codex memory at
