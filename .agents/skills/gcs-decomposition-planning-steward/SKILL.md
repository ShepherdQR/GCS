...
description: Decomposition planning design and review for GCS. Invoke when work touches context covers, overlap contexts, BoundaryProjection, GaugePolicy, subproblems, SolveDag, coverage proofs, unsupported plan reports, rigidity hints, separator planning, or local-to-global solve ordering.
---
...
When invoked for decomposition planning work:
- Use `Read` on architecture docs before designing new planning policies.
- Use `Grep` to trace how existing planners consume incidence and constraint
  indices.
- When adding a new cover strategy, verify it does not duplicate existing
  coverage logic via `Grep` on plan construction code.
- Use `Bash` for build verification after planning changes.
- Coverage proofs should reference specific contract test names.
