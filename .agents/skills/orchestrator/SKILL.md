...
**This skill is project-agnostic.** Drop it into any project's `.Codex/skills/`
...
| Context isolation is the primary value of multi-agent, not parallelism | Anthropic Harness; Codex docs |
...
project:
  ...
  skills_dir: ".Codex/skills/" # relative to root; where skills are defined
  agents_dir: ".Codex/agents/" # relative to root; where agent definitions live
...
This skill uses only standard Codex tools. No platform-specific MCP tools required.
...
- **Portability**: Project-agnostic. Requires only Codex standard tools. Configure via Project Configuration section.
