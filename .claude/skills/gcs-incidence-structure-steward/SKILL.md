...
description: Structural graph contract design and review for GCS. Invoke when work touches incidence hypergraphs, reverse indices, connected components, rigid-set or body graphs, articulation or biconnected decomposition, separators, malformed-edge reports, or deterministic graph dumps.
---
...
When invoked for incidence structure work:
- Use `Read` on architecture docs and existing graph code before modifying
  structural projections.
- Use `Grep` to find all consumers of incidence indices before changing output
  shapes.
- Verify deterministic ordering by inspecting sort keys and stable ID handling.
- Use `Bash` to run `scripts/build_clang_ninja.cmd` after structural changes.
- When adding new graph projections, use `Agent` with subagent_type="Explore"
  to verify no planner or diagnostics consumer silently depends on the old
  projection shape.
