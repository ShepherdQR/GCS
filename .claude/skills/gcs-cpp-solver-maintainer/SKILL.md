...
When invoked for C++ solver work:
- Use `Read` on `references/cpp-solver-map.md` and the target module's
  interface before editing.
- Use `Grep` to find all callers of a function before changing its signature.
- Use `Edit` for surgical C++ changes; keep module boundaries intact.
- Use `Bash` with `scripts/build_clang_ninja.cmd` after every change.
- When a change touches multiple modules, use `Agent` with subagent_type=
  "Explore" to map the full blast radius before editing.
- Never add viewer, browser, or GUI dependencies to solver code.
